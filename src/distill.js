...
export function numberingContract(eligibleCount) {
  return [
    ...
    `  ${KEEP_PREFIX} <line>,<line>,...`,
    'where each <line> is a real line number from a numbered result above.',
...
export function contractSection(minLines) {
  return [
    ...
    `  ${KEEP_PREFIX} <line>,<line>,...`,
    'Replace each <line> with a real number you actually saw, comma-separated;',
    'the bracketed number at the start of a line is the one to use. Those lines are',
    'kept verbatim and every other line is replaced by a short handle that can read',
    'the original back on demand, so dropping a line is recoverable rather than',
    'destructive. Keep the lines you would act on, quote, or cite; drop listings,',
    'progress noise, and boilerplate. Put the line at the very end of your reply.',
