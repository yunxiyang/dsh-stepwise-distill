...
/**
 * Per-request timing and throughput, keyed by `${turn}/${step}`.
 *
 * Read from the raw log, not the projection, because the projection does not
 * carry what this needs: a `request/header` never survives as a node -- only
 * `system/message`, `user/message`, `assistant/message` and `tool/result` can
 * join the surface -- and the timing lives half in that header and half in the
 * `assistant/message`'s own `stream`.
 *
 * Paired by position, not by key. `request/header` carries no turn or step (its
 * data is `header` and `reason`), so there is nothing to match on; but the two
 * events alternate in the log -- every message is preceded by the header of the
 * request that produced it -- so the nearest preceding header is the right one.
 * Matching on turn/step cannot work at all: all 933 headers collapse onto the
 * same empty key.
 *
 * Three windows, all from one pass over `stream`:
 * `first` is when the first byte of anything arrived, `reasoningEnd` closes the
 * reasoning group (`time0` plus its own `dt`, an array of per-chunk gaps),
 ...
 * subtracts them; using the full count there double-counts the thinking.
 *
 * A rate is only reported when its window is long enough to mean anything: a
 * few milliseconds divided into a few hundred tokens produces a number in the
 * thousands, which is how the `text-chunks`-only window came to disagree with
 * the provider's own throughput by an order of magnitude.
 *
 * @param session - the running session.
...
function requestTimings(session, events) {
  ...
  for (const event of events_) {
    ...
    // produced the turn's closing message. Written only when the turn has no
    // entry yet: the turn's own summary record is the request that turn cost,
    // and this one is the model call it replaced, which measures less. The
    // record is written after the message, so leaving an existing entry alone
    // keeps the summary's figures instead of overwriting them.
    if (!timings.has(`${turn}/?`)) timings.set(`${turn}/?`, timing)
  }
  return timings
}

/**
 * One message's own numbers: when its stream started and ended, and how fast
 * each half of it ran.
 *
 * Split out of {@link requestTimings} because the log holds a second kind of
 * message -- `assistant/attempt`, written when a request fails -- and its
 * `stream` has no usage and no text; the same walk has to tolerate both.
 *
 * @param event - one `assistant/message` event.
 * @param startedAt - the time of the request that produced it, or null.
 * @returns the figures, or undefined when the log cannot support them.
 */
function timingOf(event, startedAt) {
  const MIN_WINDOW_MS = 20
  ...
  for (const part of stream) {
    if (typeof part?.time === 'number') {
      if (first === null) first = part.time
      const chunk = part.chunk
      if (chunk?.type === 'finish') finish = part.time
      continue
    }
    if (typeof part?.time0 !== 'number') continue
    const span = part.time0 + sumGaps(part.dt)
    if (part.type === 'reasoning-chunks') reasoningEnd = span
    if (part.type === 'text-chunks') textStart = part.time0
  }
...
/** A count over a window, or null when the window is too short to divide by. */
function rate(tokens, windowMs) {
  if (typeof tokens !== 'number' || tokens <= 0) return null
  if (typeof windowMs !== 'number' || windowMs < 20) return null
  return tokens / (windowMs / 1000)
}

/** The gaps between streamed chunks, summed. They arrive as an array per group. */
function sumGaps(dt) {
  if (!Array.isArray(dt)) return typeof dt === 'number' ? dt : 0
  let total = 0
  for (const gap of dt) if (typeof gap === 'number') total += gap
  return total
}
...
function retentionRecords(session, timings) {
  ...
  return records.map((record) => {
    // The request that produced this record. A step record covers the step, so
...
async function requestSummary(llm, config, messages, signal) {
  ...
  // The summary call is the plugin's own request: the host never writes it to
  // the log, so its `usage` and its stream only exist here. Recording them is
...
/**
 * The usage a chunk carried, or the running value when it carried none.
 *
 * `usage` arrives on the stream rather than on the terminal frame, and some
 * hosts hand it over as a JSON string; `parseUsage` normalises both shapes.
 * Later chunks win, so folding every chunk through here lands on the same
 * figure a whole-stream scan would -- without keeping the stream around, which
 * is what the record can carry.
 *
 * @param element - one stream chunk.
 * @param current - the usage seen so far.
...
function usageFromChunk(element, current) {
  ...
  // record whose data would not round-trip through the log.
  return {
    inputTokens: numberOr(parsed.inputTokens, 0),
    outputTokens: numberOr(parsed.outputTokens, 0),
    cacheReadTokens: numberOr(parsed.cacheReadTokens, 0),
...
async function summarizeFinishedStep(agent, llm, config, signal) {
  const session = agent?.session
    ...
    const { text: summary, timing } = await requestSummary(llm, route, context, signal)
    ...
    const message = {
      ...
      // When the summary call ran and how it streamed. The host never logs the
      // plugin's own request, so without this the only timing the tab could show
      // for the step is the model's own call -- which may be a bare tool call.
      timing,
...
async function summarizeFinishedTurn(agent, llm, config, signal) {
  ...
  const { text: summary, timing } = await requestTurnSummary(llm, route, context, signal)
  if (summary.length === 0) {
  ...
  await session.append('user/message', {
    ...
    summaryOfTurn: turn,
    // When the turn's summary call ran and what it reported. Same reason as
    // the step record's: the plugin's own request is never logged.
    timing,
  }, { surfaceOp: 'append' })
