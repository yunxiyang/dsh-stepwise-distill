...
2. **选目标**:遍历 `session.surface.nodes`,取满足以下条件的 `tool/result` 节点:
   ...
   - 未被标记为已蒸馏;
   - 属于已登记编号的结果(§6.1)。
3. **找 keep 行**:在该节点之后、下一条用户消息之前的**所有** assistant 消息里查找 `keep:` 行,把它们的内容块合并后再解析。
   - **不能只看第一条**。实测(GUI 真实会话):模型在 step 4 读到结果,step 5 处理它,step 6 才写出 keep 行。只在第一条 assistant 消息里找会漏掉这个决定,而"先读、再行动、最后决定"正是常见形态。
...
`turn < 当前 turn` 这条规则意味着:**同一轮内做出的 `keep:` 决定,要等到下一轮开始才生效**。这是刻意的 —— 不碰当前轮正在被引用的节点。代价是最后一轮的结果不会被固化;收益是不必担心改写刚刚生成、可能仍被当前上下文引用的内容。
