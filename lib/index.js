...
export function apply(ctx, config) {
  ...
  ctx.logger?.info?.(`[${name}] loaded: stepSummary=${String(resolved.stepSummary)} `
    + `turnSummary=${String(resolved.turnSummary)} `
      ...
      c.effect(() => register({
        ...
        fetch: async (request) => {
          try {
            ...
            const session = sessionFor(ctx, sessionId)
 ...
 */
function sessionFor(ctx, sessionId) {
  ...
  if (typeof lookup === 'function') {
    ...
    } catch { /* fall through to the observing query */ }
  }
  return undefined
}
...
function installReasoningStrip(agent, config) {
  ...
  try {
    session.deriveMessages = () => {
      ...
      return restoreNewestStep(messages, recordsOf(session), messagesOfSeq(session))
    }
