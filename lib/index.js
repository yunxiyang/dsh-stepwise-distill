 ...
 */
export const optionalInject = ['commands', 'tools', 'llm', 'connection', 'sessionProjections']
 ...
 */
function contextBytes(ctx, session) {
  return contextTokens(ctx, session)
}

/**
 * How full the context is, as the host counts it, or null when it cannot say.
 *
 * Taken from the host's own `contextPressure` projection rather than summed
 * here. The projection is a sample of the last request's prompt-side pressure
 * plus the surface's movement since that sample, which is what the host shows
 * the reader -- so this tab and the host agree by construction. Summing message
 * bodies here would count the envelope's absence instead: every record this
 * plugin writes is also in the projection, so it would count the same text
 * twice and the copies would grow with the session.
 *
 * `projectedTokens` falls back to `pressureTokens` because the projection only
 * has the former once a pressure sample and a surface measurement both exist.
 * Either way it is a token count, not a byte count.
 *
 * @param ctx - the plugin context, for the projections service.
 * @param session - the running session.
 * @returns the context's token count, or null when the host has no answer.
 */
function contextTokens(ctx, session) {
  const projections = ctx?.sessionProjections
  if (projections === undefined) return null
  if (typeof projections.checkpoint !== 'function') return null
  if (typeof projections.viewCheckpoint !== 'function') return null
  try {
    const rows = projections.checkpoint(session)
    const view = projections.viewCheckpoint(rows, ['contextPressure'])?.contextPressure
    const tokens = numberOr(view?.projectedTokens, undefined)
      ?? numberOr(view?.pressureTokens, undefined)
        ...
        fetch: async (request) => {
          try {
            ...
            return json({
              ...
              context: contextBytes(ctx, session),
            })
          } catch (error) {
