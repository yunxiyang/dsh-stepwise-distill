 ...
 */
export const optionalInject = ['commands', 'tools']
 ...
 */
/**
 * Tools whose output already carries its own structure and must not be
 * numbered. `read` writes its own line numbers, and `history_read` returns a
 * result verbatim -- numbering a retrieval would hand the model a second,
 * conflicting index for text it is reading specifically to escape the first.
 */
export const SELF_NUMBERED_TOOLS = ['read', 'history_read']
...
/**
 * Order of the reasoning section.
 *
 * Placed just after the numbering contract so the two instructions about what
 * survives are read together, and before the host's own guidance.
 */
export const REASONING_SECTION_ORDER = PROMPT_SECTION_ORDER + 1
...
export function planDistillation(event, events, config) {
  ...
  const { found, indices, malformed, all } = parseKeep(blocks)
  // The default is DROP. A result that was never answered loses its content and
  // keeps only the retrieval handle, which is the whole point of the slot: the
  // model decides what to SAVE, and has to do nothing to let the rest go. An
  // earlier design made silence mean "keep everything", and in that shape a
  // model that never answers is never wrong -- which is why it never answered
  // (2 of 56 results across 150 measured sessions). Asking it to weigh deleting
  // is asking it to take a risk; asking it to name what it still needs is not.
  //
  // A malformed or out-of-range answer is treated as no answer, so it drops
  // too. That is deliberate: this profile's owner is in the loop and watches
  // the audit, and every dropped result is recoverable by its seq handle.
  if (all) return { skip: 'keep-all' }
  if (found && malformed) return { skip: 'malformed-keep-line' }

  // Kept line numbers, or none at all when the result was never answered.
  const requested = found ? indices : []
  ...
  const inRange = requested.filter(index => index >= 1 && index <= lines.length)
  if (inRange.length !== requested.length) return { skip: 'index-out-of-range' }
...
export function apply(ctx, config) {
  ...
  const registerHistoryRead = (tools) => {
    ...
    tools.register(defineTool({
      ...
      description: 'Read the original text of a tool result that distillation '
        + 'replaced, by the session seq from its `full: session seq N` handle.',
      parameters: {
        seq: {
          type: 'number',
          required: true,
  ...
  listen('agent/pre-step', async ({ agent, turn, step, signal }, next) => {
    ...
    installReasoningStrip(agent, resolveConfig(config))
    const decision = await next()
    const current = resolveConfig(config)
