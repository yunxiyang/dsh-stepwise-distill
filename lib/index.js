...
/**
 * The surface seq range covered by one `"turn/step"`.
 *
 * A summary replaces its step instead of being appended after it. Appending is
 * what a first version did, and it put a `user/message` at the head of every
 * following turn -- the model read each summary as a fresh thing the user had
 * just said, and answered it instead of continuing the task.
 *
 * `replace` needs the first and last surface seq of the step, and the surface
 * holds only message-bearing events, so the range is taken from the events that
 * carry `turn`/`step` on the log.
 *
 * @param events - the event log, newest last.
 * @param turn - turn number.
 * @param step - step number.
 * @param surfaceSeqs - seqs currently on the surface, in order.
 * @returns the first and last surface seq of that step, or undefined.
 */
/**
 ...
 * gone from it. Reading the surface rather than the log is therefore what keeps
 * a replacement addressable.
 *
...
function stepRange(events, turn, step, surfaceSeqs) {
  ...
  for (const event of events) {
    ...
    if (!onSurface.has(event.seq)) continue
    members.push(event.seq)
  }
  if (members.length === 0) return undefined
  return { startSeq: members[0], endSeq: members[members.length - 1], seqs: members }
}
...
async function requestSummary(llm, config, messages, signal) {
  ...
  // 2. Summarizing must not inherit the session's reasoning effort. Under
  //    `reasoningEffort: "max"` the thinking consumes the output budget and the
  //    text block comes back empty (`blocks=[{type:"text",text:""}]`). `off`
  //    must therefore be set on the call, and `call.config` then carries it.
...
async function summarizeFinishedStep(agent, llm, config, signal) {
  ...
  const message = {
    ...
    // Provenance only: it lets a reader tell which step a summary belongs to.
...
function installReasoningStrip(agent, config) {
  ...
  try {
    session.deriveMessages = () => {
      const messages = stripReasoningFrom(original())
      // Material of steps that already have a summary is dropped here, at
      // projection, so the log keeps it and only the request loses it.
      //
      // The set is read from the log, not from the claim set that guards
      // against duplicate requests: a step whose summary came back empty is
      // claimed but has nothing to replace it, and dropping its material would
      // lose the step outright.
      const summarized = summarizedSteps(readEvents(session))
      if (summarized === undefined || summarized.size === 0) return messages
      const stepOf = stepLookup(readEvents(session))
      return dropSummarizedSteps(messages, summarized, stepOf)
    }
