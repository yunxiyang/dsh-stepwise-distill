...
`
    ...
    function RecordEntry(props) {
      ...
      return createElement('div', {
        ...
        /**
     * A context size in tokens, as the thousand people read.
     *
     * Not `formatSize`: that one is for the record sizes beside it, which are
     * bytes and divide by 1024. Tokens are counted in thousands, and the two
     * numbers sit on the same line, so they cannot share a unit by accident.
     */
...
/**
    ...
    /** Seconds, one decimal: the rows are read as a column, so the unit is fixed. */
    ...
    /**
     * Tokens per second, rounded -- a tenth of a token per second is noise.
     *
     * A rate that is not there is still shown: a request with no reasoning
     * section and one whose output window was too short to measure would
     * otherwise look the same, and the row would silently lose a column. The
     * placeholder says "this request has no such figure" rather than letting
     * the reader compare two rows that are not measuring the same things.
     */
    function formatRate(rate) {
      if (typeof rate !== 'number' || !Number.isFinite(rate) || rate <= 0) return '-- t/s'
      return `${Math.round(rate)} t/s`
    }

    /**
     * A record's row label: where it belongs, how big it is, and what its own
     * request cost -- first token, then the two rates.
     *
     * The rates are per record, not running: a row describes the request that
     * wrote it. Thinking comes before the visible output because it is what
     * the request spent its time on first, and the two are separated because
     * a long think followed by a short answer reads as one slow request
     * otherwise.
     */
    function sizeLabel(item) {
      const parts = [where(item)]
      const size = formatSize(item?.size)
      if (size !== '') parts.push(size)
      const ttft = formatSeconds(item?.ttft)
      if (ttft !== '') parts.push(ttft)
      // Both rates are pushed even when the request has none: `formatRate`
      // answers `-- t/s` there, which keeps the columns aligned down the list.
      parts.push(formatRate(item?.thinkRate))
      parts.push(formatRate(item?.textRate))
      return parts.join(', ')
    }

    /**
     * The moving average over the last `window` values, oldest last.
     *
     * The newest value is the average of the newest `window` records, so the
     * head of the panel answers "how fast is this session going right now"
     * rather than "how fast has it ever gone". With fewer records than the
     * window, every record so far counts -- which makes the first record its
     * own value, the only answer available at that point.
     */
    function movingAverage(values, window) {
      ...
      for (let index = 0; index < values.length; index += 1) {
        ...
        out.push(total / slice.length)
      }
      return out
    }

    /** The average of a metric over the whole session, or null when it has none. */
    function averageOf(items, key) {
      const values = items.map((item) => item?.[key]).filter((value) => typeof value === 'number' && Number.isFinite(value) && value > 0)
